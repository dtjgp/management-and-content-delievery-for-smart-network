...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 19 11:12:43 2022
...
def statistical_result(seed, ARRIVAL, Buffersize, channelsize):
    ...
    while time < SIM_TIME:
        ...
        if event_type == "arrival":
            arrival2(time, FES, MM1, Buffersize)
            # arrival_num += 1

        elif event_type == "departure":
            departure2(time, FES, MM1)
            # served_num += 1
...
#infinite buffer
def arrival_inf(time, FES, queue): # 有两个channels,无限buffer
    global users
...
# departures *******************************************************************
def departure_inf(time, FES, queue): #两个channels
    global users
...
def statistical_result_inf(seed, ARRIVAL, channelsize):
    global users
    global load
    ...
    while time < SIM_TIME:
        ...
        if event_type == "arrival":
            arrival_inf(time, FES, MM1)
            # arrival_num += 1

        elif event_type == "departure":
            departure_inf(time, FES, MM1)
            # served_num += 1
...
for ARRIVAL in range(1, 8, 1):
    ...
    loss2_inf, avgdelay2_inf, transnum_inf, avgusernum2_inf = statistical_result_inf(42, ARRIVAL, 2)
    ...
    transnum_list_inf.append(transnum_inf)

# #plot the lossRate
# plt.figure(1)
# plt.plot(arrival_list2_3,Loss_rate_list2_3,color='red',marker='o',label='two antennas with buffersize 3')
# plt.plot(arrival_list2_inf,Loss_rate_list2_inf,color='green',marker='o',label='two antennas with infinite buffersize')
...
# plt.legend()
# plt.grid()
# plt.savefig("task2b-loss probability vs average inter-arrival time", dpi=300)
...
# plt.plot(arrival_list2_inf,avgdelay_list2_inf,color='green',marker='o',label='two antennas with infinite buffersize')
# plt.xlabel("average inter-arrival time [ms]")
...
# plt.legend()
# plt.grid()
# plt.savefig("task2b-average queuing delay vs average inter-arrival time", dpi=300)
...
# plt.plot(arrival_list2_inf,avgusernum_list2_inf,color='green',marker='o',label='two antennas with infinite buffersize')
# plt.xlabel("average inter-arrival time [ms]")
...
# plt.legend()
# plt.grid()
# plt.savefig("task2b-average user number vs average inter-arrival time", dpi=300)     
...
# plt.plot(arrival_list2_inf,transnum_list_inf,color='green',marker='o',label='two antennas with infinite buffersize')
# plt.xlabel("average inter-arrival time [ms]")
...
# plt.legend()
# plt.grid()
# plt.savefig("task2b-number of transmitted packets vs average inter-arrival time", dpi=300)
...
plt.legend()
plt.grid()
plt.savefig("task2b-average queuing delay from 1-5 ms", dpi=300)
